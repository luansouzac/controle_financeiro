              ...
              <v-list lines="two">
                ...
                <v-list-item v-for="t in incomeTransactions" :key="t.id">
                  ...
                  <template v-slot:append>
                    <span class="font-weight-bold text-h6" :class="amountColor">{{
              ...
              <v-card-text>
                ...
                <div class="d-flex align-center mt-2" v-if="!incomesStore.loading">
                  ...
                  <span class="text-body-2 ml-1" :class="`text-${comparisonColor}`">
                    <strong>{{ comparisonPercentage.toFixed(1) }}%</strong> {{ comparisonText }} que
                    o mês passado
                  </span>
                </div>
              </v-card-text>
...
const title = computed(() => (tipo.value === 'receita' ? 'Minhas Receitas' : 'Minhas Despesas'))
...
const detailsTitle = computed(() =>
  tipo.value === 'receita' ? 'Detalhes das Receitas do Mês' : 'Detalhes das Despesas do Mês',
)
...
const avatarColor = computed(() =>
  tipo.value === 'receita' ? 'success-lighten-4' : 'error-lighten-4',
)
const iconColor = computed(() => (tipo.value === 'receita' ? 'success' : 'error'))
const amountColor = computed(() =>
  tipo.value === 'receita' ? 'text-success' : 'text-grey-darken-2',
)
...
const previousMonthIncome = computed(() => {
  ...
  return incomesStore.incomesData
    .filter((income) => {
      const incomeDate = new Date(income.criado_em)
      return (
        incomeDate.getMonth() + 1 === previousMonth && incomeDate.getFullYear() === previousYear
      )
    })
    .reduce((total, income) => total + income.valor, 0)
})
...
const incomeByCategory = computed(() => {
  ...
  const currentMonthTransactions = incomesStore.incomesData.filter((income) => {
    const incomeDate = new Date(income.criado_em)
    return incomeDate.getMonth() + 1 === currentMonth && incomeDate.getFullYear() === currentYear
  })
  ...
  currentMonthTransactions.forEach((t) => {
    ...
    if (!totalsByName[categoryName]) {
      totalsByName[categoryName] = 0
    }
    totalsByName[categoryName] += t.valor
  })
  ...
  return incomesStore.incomesData
    ...
    .map((income) => {
      return {
        ...
        category: income.categoria?.nome || 'Geral',
        amount: income.valor,
        date: new Date(income.criado_em).toLocaleDateString('pt-BR', {
          day: '2-digit',
          month: 'short',
        }),
        icon: 'mdi-cash',
...
const comparisonPercentage = computed(() => {
  ...
  return Math.abs(
    ((currentMonthIncome.value - previousMonthIncome.value) / previousMonthIncome.value) * 100,
  )
})
  ...
  datasets: [
    {
      ...
      backgroundColor:
        tipo.value === 'receita' ? 'rgba(102, 187, 106, 0.2)' : 'rgba(229, 115, 115, 0.2)',
      tension: 0.4,
      fill: true,
    },
  ],
